...
5. `T-SCHOOL Schedule Sync` 自訂選單的「開啟控制臺介面」會開啟設定側欄。
6. `儲存並首次同步` 先以獨立 Apps Script 執行讀取未來 30 天課綱並計時；在 60 秒內完成時先發布快照，讓第一批 Calendar 事件直接帶入課綱，超時或失敗則不阻擋基本行程，改由背景工作補上。接著建立或選擇專用的非主要 Calendar；新日曆可自行命名，預設為「{年級}行程｜T-SCHOOL Schedule Sync」。第一次同步顯示預估時間，並以後端實際階段與已處理事件數回報進度；基本行程成功寫入後啟用觸發條件並只寄送一次「行程同步設定完成」通知。
...
- 即時通知開啟時，固定同步偵測到的行程調整會先合併並持久保存，Calendar 同步完成後立即嘗試寄出；寄送失敗時保留佇列並排程重試。關閉時，異動持續保存到下一個使用者通知時間成功寄出，後續無異動同步不得清除。通知 Trigger 若與仍在執行的同步重疊，會延後重試。同步錯誤、課綱錯誤與「行程同步設定完成」通知可立即寄發。
