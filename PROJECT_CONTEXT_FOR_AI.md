...
2. 使用者搜尋並勾選要同步的課程與活動，再設定通知 Email 與通知偏好。所有選項共用 `selectedTitles`；名稱包含全校、學習分享會、補假／補課、節假日或模考／模擬考者只套用預設勾選與置底規則，不產生課程／活動類型。「即時通知」預設開啟，偵測到行程調整並完成 Calendar 同步後盡快寄出，每日成功摘要固定 06:00；關閉後可設定一至四個通知時間，最後一個時段兼作每日成功摘要。課表偵測與 Calendar 同步以每日 03:00、11:00、18:00、21:00 為中心，在各自前後一小時內依控制臺副本分散執行，與通知設定彼此獨立。行程提醒等未顯示於安裝器的選項先採預設值，安裝後可由側欄調整；事件說明格式固定使用標準格式。
...
- 目錄只保存標題與 `period: term|vacation`；學期間／寒暑假期間只是 UI 分組，不是行程類型。只有寒暑假項目存在而形成兩組時才顯示兩個期間小標，單一期間清單不顯示泛稱「行程」小標。來源未提供時間的可解析備註行程建立為全天 Calendar 事件，不得自行創造時間。
...
- 日期改動配對會先依正規化行程標題分桶，且只允許全天型態相同、節數長度相同的事件互相配對，避免把調課判成跨節事件擴張、縮短、拆分或合併。若新舊事件都具有課綱「單元主題＋課程內容」組合身分（至少一欄有內容），兩者必須相同；相同身分優先於時間距離，實體課程教室不納入身分。缺少課綱身分時，才沿用同標題、同節數、21 日內唯一最近的貪婪配對；等距時仍不得任選，節數不同則維持分開的新增與取消。Calendar 可能仍需因連續區間邊界改變執行多筆安全更新；最終呈現給使用者的判定會再以單節交集消去「同標題、同日期、同節次、同地點」的未變動部分，避免把純簡分組邊界改變多報成一項調課。
...
- 課綱只在已選行程的原始標題與工作表分頁名稱一字不差時附加，再以日期與節次起訖配對；不得使用分頁內「中文名稱」、標題正規化或模糊比對作 fallback。
- 沒有精確對應分頁代表該行程不附加課綱，不用來推斷行程類別，也不列為缺頁錯誤；基本行程仍正常同步。
...
- 課綱快照只保存已選、標題精確對應課綱分頁，且落在今天至第 30 天（含頭尾）的行程。純非同步列略過；同時具有實體或線上同步時數的混合列保留。整學期基本行程仍照常同步，不受 30 天課綱視窗影響。
...
- 明確的同標題、同節數日期或時間變更，會在 21 天範圍內配對為更新；課綱的「單元主題＋課程內容」組合可作為跨時間與地點的事件身分，其中一欄留空時仍使用另一欄已提供的資訊。節數不同、課綱身分明確衝突或其他不明確情況維持分開的新增與取消；但最終異動摘要會先消去連續區間重新分組後仍落在原時段的單節，只呈現真正換位的節次。
...
- 事件提醒預設關閉，可由使用者設定。
- 推定新學期時，系統暫停觸發條件、在完成新學期同步前不改動既有 Calendar 事件，並把 `selectedTitles` 重設為新學期的預設選項（全校、學習分享會、補假／補課、節假日、模考／模擬考）供使用者確認。側欄持續顯示重新選擇課程與活動的警示；Email 以學期識別去重，第一次寄送失敗會在約 30 分鐘後重試一次。使用者必須明確確認新學期實際就讀年級並至少選擇一項課程或活動，儲存後才恢復寫入；側欄預設恢復轉換前的自動同步偏好。若自動同步保持關閉，使用者完成手動同步後仍排定一次使用 `manual` 路徑的課綱更新，但不建立每日觸發器。
...
- 每份 Google Docs 控制臺副本支援一個年級、一個通知信箱與一個專用 Calendar；拒絕使用主要 Calendar。通用程式的 `DEFAULT_SETTINGS` 不得內含測試使用者 Email、行程選擇、Calendar ID 或預設年級；匯入前也不得讀課表、建立 Calendar、建立觸發器、同步、修復或刪除。
