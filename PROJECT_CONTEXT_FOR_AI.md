...
6. `儲存並首次同步` 先以獨立 Apps Script 執行讀取未來 30 天課綱並計時；在 60 秒內完成時先發布快照，讓第一批 Calendar 事件直接帶入課綱，超時或失敗則不阻擋基本行程，改由背景工作補上。第一次同步顯示預估時間，並以後端實際階段與已處理事件數回報進度：0–35% 表示讀取與規劃，35–90% 依 Calendar 操作完成比例推進，92–99% 表示狀態、排程、通知與清理；只有工作狀態與背景清理完成後才回報 100%。基本行程成功寫入後啟用觸發條件並只寄送一次「行程同步設定完成」通知。
