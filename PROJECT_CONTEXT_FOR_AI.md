...
2. 使用者搜尋並選擇課程與活動、排除個別活動，並設定通知 Email 與一至四個通知時間；最後一個時段同時作為每日成功摘要時間，讓當日行程調整優先。活動提醒等未顯示於安裝器的選項先採預設值，安裝後可由側欄調整；事件說明格式固定使用標準格式。
3. 安裝器產生一份 `Code.gs`。
...
- 每日行程同步觸發器使用 `atHour(hour).nearMinute(0)`，由 Apps Script 在所選整點前後約 15 分鐘啟動；實際通知須等該次同步完成後寄出。非最後通知時段使用排程專用入口，最後一個時段使用含每日成功摘要的入口，兩者都直接標記為通知時段，避免提早於整點啟動時被誤判成時段外。課綱每日更新仍只指定小時、不固定分鐘，以分散共用 Sheets 的讀取尖峰。
...
- Calendar 寫入批次本身不直接開啟課綱 Sheets，只讀取最後成功快照；第一次同步前的課綱預讀也在獨立 Apps Script 執行完成。課綱每天會自動重新讀取，時間安排在最早自動同步時段的約 2 小時前；第一次失敗於約 30 分鐘後重試，第二次仍失敗才寄信，硬逾時由預先建立的 watchdog 接手。
...
- 一般通知只在使用者設定的一至四個通知時間寄發；時間外偵測到的行程調整與需確認事項先排入佇列。同步錯誤、課綱錯誤與「行程已開始同步」通知可立即寄發。
