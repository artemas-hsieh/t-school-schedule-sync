...
偵測到新學期時，程式會先停止自動改動 Calendar，並要求重新確認年級與選課。請依序：

1. 開啟控制臺，若已升年級，先切換至新學期實際就讀年級。
2. 勾選「我已確認這是新學期就讀年級」。
3. 重新選擇至少一門課程。
4. 選擇「完成選課並同步」，檢查新增、更新與移除預覽後再確認。

完成新學期同步前，現有 Calendar 行程不會被改動；同步後，未出現在新選課中的未來行程會依預覽結果更新或移除，過去行程仍會保留。若自動同步原本關閉，這次手動同步仍會另外排定一次課綱更新；之後要再次更新，需再執行手動同步。
