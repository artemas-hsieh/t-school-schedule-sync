...
1. 開啟線上設定器。
2. 選擇年級，等待課表讀取完成。
3. 網站依課程索引的年級分頁列出課程，必修預設勾選、其他預設不勾選；選好課程後填寫通知 Email 與通知偏好。
4. 檢查設定後按下「產生設定碼」。
...
若你直接在 Google Calendar 刪除仍在課表中的受管理行程，系統會在下次同步先補回，並寄信提醒。請用電腦開啟行程同步控制臺，在「同步狀態」區段選擇「刪除單一事件」或「刪除整門課程」。刪除單一事件會直接執行；刪除整門課程時會再要求確認。這些動作只處理本控制臺已記錄且管理標記相符的事件，同日曆中的私人事件不會受影響。
...
完成新學期同步前，現有 Calendar 行程不會被改動；同步後，未出現在新行程選擇中的未來行程會依預覽結果更新或移除，過去行程仍會保留。課綱內容與時間會在同一次主同步讀取；關閉自動同步後仍可手動同步。

所有使用者均使用 Google Docs 控制臺；課程目錄取自公開課程索引，實際行程取自課綱 Sheets，不再讀取公開課表 API、全校活動與課表備註

當前年級該學期全部課程結束，且下一學期課綱已啟用並完整可讀時，才進入新學期確認；可在寒暑假提前設定，不必等適用起日

新增學期方式、解析保護及原控制臺更新步驟見 [課綱來源指引](docs/sheet-source.md)；本機完成不代表正式 Google Docs 母版或網站已發布
