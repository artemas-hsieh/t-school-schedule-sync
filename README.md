...
所有使用者均使用 Google Docs 控制臺，課表 Sheet 與 Sheet 控制臺已停止使用；課程大綱 Sheets 仍是獨立的補充來源

新的私人課表擷取與發布系統正在進行第一階段本機驗證，正式 API 尚未切換；部署條件、Google Docs 原副本更新方式與待驗收項目見 [來源更新部署指引](docs/source-publication.md)
