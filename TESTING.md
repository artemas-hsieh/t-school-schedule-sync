...
- 課表偵測與 Calendar 同步固定建立 03:00、11:00、18:00、21:00 四個 `atHour(hour).nearMinute(0)` Trigger，不得隨通知時間改變；使用者選擇的每個通知時間則建立只負責寄信的獨立 Trigger。
- 一般通知在設定時間外只排入佇列；固定同步後若沒有新異動，不得清除先前待寄的行程調整。通知 Trigger 與背景同步重疊時必須延後重試；錯誤與「行程已開始同步」通知可立即寄出。
...
- 受影響的主要互動流程。
- 第四步以前修改年級、課程、Email 或通知時間時，`Code.gs` 不得即時產生或更新；只有按下第四步「產生安裝程式碼」後才更新，且複製控制在產生前保持停用。
- 鍵盤操作與合理的焦點順序。
- focus 狀態是否可見。
- 表單驗證、錯誤訊息與狀態轉換。
- 在第三步 Email 無效時按 Enter，確認欄位自動聚焦、卡片內提示可被輔助技術辨識，且不出現瀏覽器原生驗證浮窗；輸入至有效格式時，第四張預覽卡片不得出現單幀白色閃爍。
- reduced-motion fallback。
- 觸控使用情境。
- 保持 Lenis 開啟，從第四步首次產生程式碼進入第五步，再分別透過手動捲動與階段選單往返第四、五步；確認手動捲動具有平滑效果但不會停住後突然追趕，階段選單仍平滑定位，active step 與 `inert` 狀態正確。將游標放在程式碼預覽上捲動時，必須沿用相同的頁面平滑捲動，不得切換成原生捲動或內嵌捲動。第五步滑至文件底部後再做數次短距離向下滑動並放手，確認平滑不會突然消失、畫面不會抽搐或回彈。
