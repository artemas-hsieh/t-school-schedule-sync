...
- 測試高一、高二、高三資料。
- 比較安裝器與產生的 Apps Script 所得到的單一行程目錄、學期間／寒暑假期間分組、事件數量與日期範圍。目錄列只能包含標題與期間，不得出現新的課程／活動類型。
...
- 行程原始標題與工作表分頁名稱在統一全半形、移除所有空白並忽略大小寫後相同時才附加課綱；不使用分頁內中文名稱，也不對其他差異做模糊比對。沒有正規化後對應分頁的行程不附加課綱、不推斷類別、不回報缺頁錯誤，基本行程仍正常同步。
- 網站與控制臺都不顯示 `自然進階(二)`；任選 `自然進階(二)_化學`、`自然進階(二)_生物`、`自然進階(二)_物理` 至少一項後，設定與事件篩選會自動包含共同事件，且每個分科事件都使用 `自然進階(二)` 分頁課綱並保留自己的原始事件名稱。
- 欄位換序、增欄，以及標頭位於不同列時，仍能依欄名定位。
- 單節、一般連續範圍，以及 `56`、`234`、`5678`、`234567`、`12345678` 等連續節次連寫都能解析；三碼以上不連續的節次不任意展開。
- 日期配對忽略來源年份，只依月／日；西元年、民國年、兩位數年份與無年份格式都能命中唯一候選。同一候選集合若出現兩個相同月／日則不配對，錯誤月／日也不得被年份忽略規則掩蓋。
...
- 每次課綱刷新只處理今天至第 30 天（含頭尾）內、已選且有正規化後對應分頁的行程；第 31 天以後不讀取課綱，但基本課表行程仍應存在。
- 第一次同步的課綱預讀必須使用獨立 Apps Script 執行；只有 60 秒內完成的快照可供第一批 Calendar 事件使用，超時或失敗不得阻止基本行程同步。
- 學期識別必須是「課表年級｜學年度-學期」：8–12 月與翌年 1 月屬同一第 1 學期，2–7 月屬前一年度第 2 學期。同學期內首日改變不可觸發轉換；舊版日期型 `termKey` 必須正規化且 v2 指紋仍可驗證。
- 第一次看到不同學期時只建立 30 分鐘候選，不清空選擇或寄信；同一學期與完整課表指紋穩定至到期才確認。指紋變動應重新計時，回到原學期應清除候選並恢復觸發器，來源失敗時保持凍結並在連續第二次才通知。一般同步若剛好在候選到期後完成確認，該次執行也必須以需處理狀態中止，不得繼續建立 Calendar 工作單。
- 確認轉換後才把 `selectedTitles` 重設為當期預設勾選項目、暫停自動同步，並保持 Calendar 凍結至使用者完成新學期選擇；側欄驗證中停用儲存、同步與修復，確認後持續顯示重選警示。使用者必須確認實際就讀年級並選擇至少一項才能儲存或同步。
- 新學期 Email 必須使用專用一次性觸發器：即時通知模式排在下一個 06:00，否則排在下一個自訂時段。排入觸發器不得寫入 `sentAt` 或增加寄送次數；只有 `MailApp` 成功後才標示已寄送。處理器開始時先移除已消耗的同名觸發器；鎖競爭必須重新排程，來源讀取或寄信失敗則保存錯誤並建立 30 分鐘重試，達上限後清除排程時間並在控制臺顯示失敗。使用者先儲存新學期設定時清除待寄狀態與觸發器。
- 控制臺初次顯示時，課綱狀態只能使用已傳入的最後成功索引或內建 fallback，不得在狀態建構函式內再次開啟中央索引；使用者主動切換年級時則須重新讀取最新索引，並一併刷新課表、課綱狀態與學期日期。
- 自動同步關閉時，一般儲存不得自行更新課綱；使用者按下手動同步或「完成選擇並同步」後，必須以獨立的一次性 `manual` 課綱觸發器補入課綱，且不得重建每日同步觸發器。
