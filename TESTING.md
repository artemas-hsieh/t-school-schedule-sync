...
- 觸控使用情境。
- 保持 Lenis 開啟，從第四步首次產生程式碼進入第五步，再分別透過手動捲動與階段選單往返第四、五步；確認手動捲動具有平滑效果但不會停住後突然追趕，階段選單仍平滑定位，active step 與 `inert` 狀態正確。第五步滑至文件底部後再做數次短距離向下滑動並放手，確認平滑不會突然消失、畫面不會抽搐或回彈。
