...
1. 先用行程原始標題，一字不差對應 Google Sheet 分頁名稱。
2. 再以日期與起訖節次配對。
3. 不使用分頁內的「中文名稱」。
4. 不使用模糊比對自動猜測。

只有標題與工作表分頁名稱完全一致的行程才會附加課綱。找不到完全一致的分頁時，該行程保留基本課表內容、不附加課綱，也不據此推測行程類別或回報缺頁錯誤；相似名稱同樣不會自動配對。
...
2. 確認需要附加課綱的行程，其工作表分頁名稱與課表原始行程標題完全一致。
3. 確認校內使用者帳號都有課綱 Sheets 與中央索引的唯讀權限。
4. 同一來源組的每一列使用相同年級、來源組鍵及適用起訖日；每份 Sheet 各占一列，連結不可重複。
5. 完成前四項後啟用完整的新學期來源組，立即以測試帳號確認索引來源為 `live`，並確認標題完全一致的測試行程能更新課綱；沒有同名分頁的行程應維持基本事件且不報錯。
