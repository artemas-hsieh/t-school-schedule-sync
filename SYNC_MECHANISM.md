...
這份說明以普通高中學生能理解為目標，同時保留足以供維護者檢查的技術細節。
...
- 每批最多 40 次 Calendar 操作。
- 每批另設 150 秒的軟性時間上限；即使未滿 40 次，時間先到也會先存檔。
- 尚未完成時，約一分鐘後由唯一的背景觸發器接著做。
- 422 筆首次同步最多約 11 批；時間較長，但每一批都遠離危險邊界。
...
flowchart TB
  ...
  L --> M["先保存本批 in-flight 清單與 watchdog"]
...
活動只依明確規則辨識，例如全校活動、模擬考、防災演練等。程式不會因為某個名稱不在選課清單，就擅自猜它是活動。
...
儲存設定時，程式會把學生端的名稱重新對回來源中的正式顯示名稱，不把含多餘空格的版本永久保存。
...
為避免 Script Properties 使用多個學期後持續膨脹，超過 120 天的「內部同步索引」會移除，但 Calendar 上的歷史事件仍原封不動保留。也就是丟掉舊書的盤點卡，不是把書從書架移除。
...
目前只有「114-2 高二」設定四份來源。未來新增課綱時，只需增加新的年級、學期代號、適用起訖日與 Spreadsheet IDs；舊課綱不會只因月份剛好相同而誤用到新學期。
...
它不覆蓋 Google Calendar 的地點欄。老師只修改課綱時，程式只更新說明，也不把它列為「課表異動」寄信。
...
- 第一次失敗約 30 分鐘後重試。
- 第二次仍失敗才寄信。
- 課綱工作硬逾時時，由獨立 watchdog 轉入相同重試流程。
...
所有更新與刪除都先驗證這個管理標記。若學生移除標記，系統會保留該事件並停止，而不把它當成仍可任意修改的受管理事件。
...
一般同步若簽章未變，就不讀寫該 Calendar 事件。因此學生自己做的非來源修改，不會每天被無故覆蓋；「強制修復」才會逐筆驗證來源管理欄位。
...
2. 讀取現有工作單；若是背景續跑，先保存新的心跳並建立 watchdog。
3. 重新驗證設定、課表與課綱版本。
4. 用目前已提交狀態重算剩餘差異。
5. 選出最多 40 次操作。
6. 先把這 40 次保存為 `inFlight`。
7. 建立 watchdog。
8. 依序更新、建立，最後才刪除。
9. 保存新的事件 ID 與狀態。
...
- 下一次仍在相同事故中硬逾時：停止並寄信。
- watchdog 自己拿不到鎖：延後再檢查，不和仍在工作的批次搶寫。

批次保存與建立續跑觸發器之間，watchdog 會一直保留；確認續跑觸發器存在後才移除，避免工作卡在「有進度、沒人接手」的狀態。
...
每位學生會用自己的校內帳號與自己的 Apps Script、Calendar、Properties 執行，因此 48 或 144 人不會把所有 Calendar 寫入集中在同一個中繼帳號。
...
- 每日觸發器只指定小時、不固定分鐘；依 [Apps Script 官方觸發器說明](https://developers.google.com/apps-script/guides/triggers/installable#time-driven_triggers)，Google 會在該小時內選一個略為隨機、之後每日保持一致的時間，避免所有學生卡在整點同時開始。
...
正式上線前仍需用校內帳號重新產生並貼上新版 `Code.gs`，在隔離測試 Calendar 做一次真正的 422 筆分批整合測試。這一步才能驗證 Google 當時的實際資源調度、觸發器延遲與權限狀態；本機 mock 無法代替 Google 服務本身。
