...
- 首次設定的前 40 筆成功保存後寄送「首批 40 筆同步完成｜T-SCHOOL Schedule Sync」通知，明確說明其餘事件仍在背景同步；基本行程全部寫入後寄送「行程已開始同步｜T-SCHOOL Schedule Sync」。
- 來源異動摘要只在整份工作完成後寄一次。
- 純課綱說明更新不列入行程調整信。
- 「行程已開始同步」信只在所有基本事件完成後寄；課綱是否已帶入仍以控制臺的課綱更新狀態為準。
