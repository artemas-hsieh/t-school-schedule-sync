...
2. 確認每個課程的工作表分頁名稱與課表原始課程名稱完全一致。
3. 確認校內使用者帳號都有課綱 Sheets 與中央索引的唯讀權限。
4. 同一來源組的每一列使用相同年級、來源組鍵及適用起訖日；每份 Sheet 各占一列，連結不可重複。
5. 完成前四項後啟用完整的新學期來源組，立即以測試帳號確認索引來源為 `live`、課綱更新成功且缺少分頁為零。
...
任何同步只要將移除目前受管理未來事件的 40%（含）以上就會停止；不設最低筆數，因此小樣本的全刪或半數刪除也不會繞過。比例的分母只計未來事件，不讓大量歷史資料稀釋風險；更換專用 Calendar 時從舊 Calendar 清理的整批受管理未來事件亦納入同一閘門。

在計算刪除前，系統先驗證來源年級、每週七個真實日期、八個節次、日期順序及週次對應，並將同學期的未來日期／週次與最後成功同步後保存的 accepted baseline 比較。唯讀 UI 快取不會更新 baseline。來源範圍退縮或學期識別與重疊日期矛盾時不可核准，Calendar 保持不變。

結構可信但沒有未來日期時標記為 expired；使用者仍可在控制臺預覽後單次確認。expired 與大量刪除可以由同一份一次性核准涵蓋。

控制臺預覽建立的 approval 綁定操作入口、設定、Calendar、完整來源版本、日期涵蓋範圍、舊 state、目標事件與刪除清單：

- 10 分鐘內且所有 fingerprint 仍相符：允許執行一次。
- 來源、設定、state、Calendar 或刪除清單改變：approval 失效。
- 同一次 approval 已建立同步工作單：立即消耗，不能重播；背景續跑改由工作單保存的核准結果繼續。

自動同步沒有 approval，因此遇到 expired 或 40% 刪除時保留日曆並要求使用者開啟控制臺。立即同步與強制修復都先走相同預覽，不能直接繞過保護。
...
- 在側欄持續顯示警示；使用者未明確確認新學期就讀年級，或未選任何課程前，不可儲存或執行同步。
- 等使用者開啟控制臺，先確認升年級後的實際年級，再重新選課；儲存時預設恢復轉換前的自動同步偏好。
- 使用者完成同步前會看到新增、更新與移除預覽；同步後，未出現在新選課中的未來行程會依確認結果更新，過去行程不受影響。
- 即使轉換前已關閉自動同步，使用者按下「完成選課並同步」後仍會排定一次手動課綱更新；後續不會自行建立每日觸發器。
