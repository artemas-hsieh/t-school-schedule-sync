...
因此不只是「行程標題清單相同」就算同一版本；日期、節次或課表基地改變仍會產生新的來源指紋。來源左欄鐘點不納入指紋。
...
第 1–8 節固定對應 `08:25–09:15`、`09:15–10:05`、`10:15–11:05`、`11:05–11:55`、`13:25–14:15`、`14:15–15:05`、`15:15–16:05`、`16:05–16:55`，程式不讀取左欄鐘點。跨節行程取首節開始與末節結束建立單一連續區間，不因下課或午休拆分；同日期、同標題、同基地且重疊或相鄰的來源格會先合併。

系統不再將來源分成課程或活動，也不以名稱關鍵字、出現節數或是否有課綱推測類別。來源有明確日期、但沒有節次的可解析備註，會建立為全天事件。
